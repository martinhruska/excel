...
It is sometimes done during the backward run between a forest automaton from forward run before
an abstract transformer application and the last forest automaton from backward state.
The resulting forest automaton is then further used in backward run after an applcation of
of the related reverse abstract transformer.
This is illustrated by Figure \ref{fig:bwrun}.
Consider an oval representing a language $L(X_{k+1})$ which is the language
of a forest automaton in backward.
The next state in backward run consisting language $L(X_{k})$ of a forest automata is
get by application of the reverse abstract transformer (illustrated by the doted lines) on the intersection
of backward run state language $L(X_{k+1})$ and forward run state language $L(M^{\alpha}_{k+1})$.
It is not necessary to do the intersection before each reverse of an abstract transformer in Forester
but it is neede for a few of them.
...
The realization of intersection is not only usefull for backwar run but
