...
\begin{figure}[t]
	...
	\caption{
		...
		Figure shows a forest automata langauge before (green area)
...
rising from a possible unboundness of the mentioned dynamic data structures.
...
which avoids repreating of the abstraction of the states
...
We mentioned that it could be neccessary to perform a forest automata intersection during
the backward run.
But it is not neccessary always because Forester implements many abstract transformers
...
from the backward and the forward run what brings better performence of the backward run.
An example where the intresection is neccessary to be done is for example an operation for freeing a memory node.
...
Morever, as it was already mentioned the intersection of foster automata is needed in some reverse
...
We mentioned that this work implementes either backward run and predicate abstraction only for
non-hiearchical forest automata.
The extension of them for hiearchical forest automata is not part
...
Moreover, the $3$ spurious counterexamples have been dected in heap manipulation category.
...
Note that Forester does not currently successfuly process all programs in the test set
...
\begin{table}[h]
	...
	score in SV-COMP competitionl. The left column gives a number of the points before backward run implementation 
...
(a name of a method in Forester differs from the name of the semanticaly same method in VATA)
...
able to analyse more real word programs.
