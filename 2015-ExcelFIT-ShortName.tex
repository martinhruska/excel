...
\begin{figure}[t]
	...
	\caption{
		...
		and forward (the green and yellow areas) run is empty at level of the symbolic state with language $L(M^{\alpha}_{k-1})$.}
	\label{fig:bwrun}
\end{figure}
...
and moreover, not even termination of verification procedure is guaranteed.
...
Another effect of the abstraction is the overapproximation of the reachable
...
Forester now implements the so called height abstraction.
...
The main problem of this abstraction technique is possibility of its fine tuning
because the only way how to refine it is by the parameter defining the depth of tree comparison.
...
it has not been yet implemented and it realization would enrich the whole forest automata theory.
...
(represented by the red and the orange area) of backward run state language $L(X_{k+1})$
and forward run state language $L(M^{\alpha}_{k+1})$.
It is not necessary to do the intersection before each reverse of an abstract transformer in Forester
