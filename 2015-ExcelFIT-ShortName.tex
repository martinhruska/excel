...
\Abstract{
  ...
  In the area of shape analysis there exists a tool Forester based on forest automata.
...
However, Forester does not currently use the mentioned predicate abstraction but \emph{height abstraction} now %TODO
...
what does not ensure that the found spurious error will not be found again.
...
The next state in backward run with forest automata language $L(X_{k})$ is obtained
...
So the first thing, that was done, was the reduction of number of the dependencies between classes,
