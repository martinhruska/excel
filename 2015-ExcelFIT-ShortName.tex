...
This problem could be solved by introducing so-called \emph{boxes} \cite{forester13}.
...
More precisely, boxes can be viewed as forest automata representing repeating subgraphs.
So it is still possible to represent the hypergraph with forest automata where
boxes are also symbols of forest automata.
...
Note that a forest automaton of certain level can contain in it alphabet only automata of lower level then its own level.
...
\begin{figure}[t]
	...
	\caption{
		...
		Figure shows a forest automaton language $L(M_i)$ before (green area)
		and language $L(M_i^{\alpha})$ after (yellow area) abstraction where
		$i$ is the order of the state within symbolic execution.
		...
		by abstract transformers during symbolic execution what is illustrated by lines connecting ovals.
...
and moreover, not even termination of verification procedure is guaranteed at all.
...
But the abstraction also includes the overapproximation of the reachable
states of an analyzed system.
This can lead to reaching the error states in
...
what avoids repeating of the abstraction of the states
that lead to the spurious error.
The employment of abstraction over forest (and tree) automata in this work is based on \emph{abstract tree regular model checking} \cite{artmc}
...
The main problem of this abstraction technique is low ability of refinement.
The only way how to refine it is by the parameter defining the depth of tree comparison
what does not ensure that the found spurious error would not be found again.
...
Finding the new predicate and checking spuriousness of an error is done by performing \emph{backward run}.
A~backward run starts from forest automata modelling a heap with a found error and
...
that is neccesary for some reverse abstract transformers.
...
it has not yet been implemented and it realization would enrich the whole forest automata theory.
The intersection is sometimes needed to be done during the backward run between a forest automaton from forward run
...
of a forest automaton in backward and an abstract transformer $\beta$ such that $\beta(L(M^{\alpha}_{k})) = L(M_{k+1})$.
The next state in backward run with forest automata language $L(X_{k})$ is
get by an application of the reverse abstract transformer $\beta^{-1}$ (illustrated by the doted lines) on the intersection
(represented by the red and the orange area) of language $L(X_{k+1})$ from backward run 
and language $L(M^{\alpha}_{k+1})$ from state forward run.
It is not necessary to do the intersection before each reverse abstract transformer application in Forester
...
As it was said, the realization of intersection is not only usefull for backward run but
...
So the first thing needed to be done is reducing number of dependencies between classes,
